...
class IntradayDataUpdater:
    ...
    def __init__(self, intraday_dir: Path = None):
        self.intraday_dir = Path(intraday_dir) if intraday_dir else Path(INTRADAY_DIRECTORY)
    ...
    def get_window_dates(self) -> Tuple[datetime, datetime]:
        ...
        Returns:
            ...
            - window_start: today - 61 days (00:00:00)
            - window_end: today - 31 days (23:59:59)
        """
        today = datetime.now().date()
        
        start_date = today - timedelta(days=WINDOW_START_OFFSET)
        end_date = today - timedelta(days=SEBI_LAG_DAYS)
        
        window_start = datetime.combine(start_date, time(0, 0, 0))
        ...
        try:
            ...
            if len(df_filtered) < original_len:
                if df_filtered.empty:
                    ...
                    file_path.unlink()
                    logger.info(f"Deleted {file_path.stem} (all data older than window)")
                    return True
                
                # Save filtered data
                df_filtered.to_parquet(file_path, compression='snappy')
                logger.info(f"Pruned {file_path.stem}: {original_len - len(df_filtered)} old rows removed")
                return True
    ...
    def filter_to_window(self, df: pd.DataFrame, window_start: datetime, 
    ...
    def fetch_intraday_data(self, symbol: str, window_start: datetime,
                            window_end: datetime) -> Optional[pd.DataFrame]:
        """
    ...
    def update_intraday_file(self, symbol: str, window_start: datetime,
                             window_end: datetime, min_rows: int = 0) -> bool:
        """
    ...
    def validate_intraday_file(self, file_path: Path, window_start: datetime,
        ...
        try:
            ...
            df.columns = [col.title().replace('_', '') for col in 
    ...
    def run_update(self, symbols: List[str] = None, max_symbols: int = None,
                   min_rows: int = 0, sleep_seconds: float = 1.0,
                   prune_old: bool = True, validate: bool = True) -> Dict:
        """
        ...
        logger.info(f"Window start: {window_start.date()} (today - {WINDOW_START_OFFSET} days)")
        ...
        for symbol in symbols:
            ...
            success = self.update_intraday_file(symbol, window_start, window_end, min_rows)
        ...
        if validate:
            ...
            files = self.get_all_intraday_files()
            for file_path in files:
                result = self.validate_intraday_file(file_path, window_start, window_end)
        ...
        logger.info(f"Validation errors: {len([r for r in validation_results if not r['valid']])}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Minimum rows required to save file'
    )
    parser.add_argument(
        '--sleep-seconds',
        type=float,
    ...
    parser.add_argument(
        '--no-prune',
        action='store_true',
        help='Disable pruning of old data'
    )
    parser.add_argument(
        '--no-validate',
        action='store_true',
        help='Disable validation after update'
    )
    parser.add_argument(
        '--intraday-dir',
        type=str,
