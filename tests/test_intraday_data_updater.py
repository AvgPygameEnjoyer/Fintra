...
class TestWindowCalculations:
    ...
    def test_window_start_is_61_days_ago(self, updater):
        """Test window start is today minus 61 days."""
        window_start, window_end = updater.get_window_dates()
        expected_start = datetime.now() - timedelta(days=WINDOW_START_OFFSET)

        diff = abs((window_start - expected_start).total_seconds())
        assert diff < 86400  # Within 1 day

    def test_window_end_is_31_days_ago(self, updater):
        """Test window end is today minus 31 days (SEBI compliance)."""
        window_start, window_end = updater.get_window_dates()
        expected_end = datetime.now() - timedelta(days=SEBI_LAG_DAYS)

        diff = abs((window_end - expected_end).total_seconds())
        assert diff < 86400  # Within 1 day
    ...
    def test_window_spans_30_days(self, updater):
        ...
        assert duration == WINDOW_DAYS + 1  # 31 days inclusive
    ...
    def test_window_end_is_end_of_day(self, updater):
        """Test window end is at 23:59:59."""
        _, window_end = updater.get_window_dates()

        assert window_end.time() == time(23, 59, 59)
...
class TestUpdateIntradayFile:
    ...
    def test_updates_file_with_new_data(self, mock_fetch, updater, temp_intraday_dir):
        ...
        with patch('scripts.intraday_data_updater.get_intraday_parquet_path') as mock_path:
    ...
    def test_respects_min_rows_requirement(self, mock_fetch, updater):
        ...
        mock_data = pd.DataFrame({
            'Open': [100],
            'Close': [103]
        }, index=pd.DatetimeIndex([window_end]))
        mock_fetch.return_value = mock_data

        result = updater.update_intraday_file('TEST.NS', window_start, window_end, min_rows=100)
...
class TestValidateIntradayFile:
    ...
    def test_detects_missing_columns(self, updater, temp_intraday_dir):
        ...
        df = pd.DataFrame({
            'open': [100],
            'close': [103]
        }, index=pd.DatetimeIndex([window_start]))
        df.to_parquet(file_path)
...
class TestRunUpdate:
    ...
    def test_prunes_old_data_when_enabled(self, populated_updater):
        ...
        with patch.object(populated_updater, 'prune_all_old_data', return_value=5) as mock_prune:
            populated_updater.run_update(symbols=['A.NS'], prune_old=True, validate=False)
    ...
    def test_skips_prune_when_disabled(self, populated_updater):
        ...
        with patch.object(populated_updater, 'prune_all_old_data') as mock_prune:
            populated_updater.run_update(symbols=['A.NS'], prune_old=False, validate=False)
...
class TestEdgeCases:
    ...
    def test_handles_file_with_timezone_data(self, updater, temp_intraday_dir):
        ...
        dates = pd.date_range(start=window_start, periods=10, freq='5min', tz='Asia/Kolkata')
        df = pd.DataFrame({
            'Open': range(10),
            'Close': range(10)
        }, index=dates)
